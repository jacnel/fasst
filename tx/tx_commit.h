...
forceinline void Tx::send_updates_to_replicas(coro_yield_t &yield,
  ...
  tx_dassert(replica_vec.size() >= 1 && replica_vec.size() <= HOTS_MAX_BACKUPS);

  size_t req_i = 0;
  rpc->clear_req_batch(coro_id);

  // Not ideal, but we need response buffers for all invalidation messages.
  bool **inval_resp = new bool *[write_set.size()];

  for (size_t w_i = 0; w_i < write_set.size(); w_i++) {
    tx_rwset_item_t &item = write_set[w_i];

    //+ Invalidate the accessors.
    //
    // We must invalidate every item that we are attempting to write to. That
    // way no reader will use a stale copy.
    std::vector<uint32_t> accessors = get_accessors(&item.dir_entry);
    inval_resp[w_i] = new bool[accessors.size()];
    uint32_t a_i = 0;
    for (auto a : accessors) {
      rpc_req_t *req = rpc->start_new_req(
          coro_id, RPC_CACHE_REQ, a,
          reinterpret_cast<uint8_t *>(&inval_resp[w_i][a_i++]), sizeof(bool));
      tx_dassert(req_i < RPC_MAX_MSG_CORO);
      tx_req_arr[req_i] = req;
      req_i++;

      size_t size_req =
          cache_forge_inval_req(req, coro_id, item.key, cache_reqtype_t::inval);
      req->freeze(size_req);
    }
    ...
    for (int repl_i : replica_vec) {
      ...
      int repl_mn =
          (repl_i == 0) ? item.primary_mn : item.backup_mn[repl_i - 1];
      uint16_t rpc_reqtype = item.rpc_reqtype + repl_i;

      rpc_req_t *req = rpc->start_new_req(coro_id, rpc_reqtype, repl_mn,
                                          (uint8_t *)item.obj->val,
                                          sizeof(uint64_t)); /* Small resp */
      ...
      if (item.write_mode != tx_write_mode_t::del) {
        /* Insert or update */
        size_req =
            ds_forge_generic_put_req(req, caller_id, item.key, item.keyhash,
                                     item.obj, ds_reqtype_t::put);
      } else {
        /* Delete */
        size_req = ds_forge_generic_get_req(req, caller_id, item.key,
                                            item.keyhash, ds_reqtype_t::del);
      }

      // Update cache.
      tx_dassert(item.is_cached);
      Cache *cache = cache_mgr->get_cache(item.rpc_reqtype);
      if (item.write_mode != tx_write_mode_t::del) {
        CacheResult result = cache->commit_update(
        ...
        //
        // If the update fails, then the item was evicted. Should we consider
        // this case?
        tx_dassert(result == CacheResult::kSuccess);
      } else {
        CacheResult result = cache->commit_del(
            coro_id, item.keyhash, reinterpret_cast<const char *>(&item.key),
            sizeof(hots_key_t));
        tx_dassert(result == CacheResult::kSuccess);
      }
  ...
  for (size_t _req_i = 0; _req_i < req_i; _req_i++) {
    uint16_t resp_type = tx_req_arr[_req_i]->resp_type;
    _unused(resp_type);
    tx_dassert(resp_type == (uint16_t)ds_resptype_t::put_success ||
               resp_type == (uint16_t)ds_resptype_t::del_success);
  }

  // Release ownership of the item.
  for (size_t w_i = 0; w_i < write_set.size(); ++w_i) {
    tx_rwset_item_t &item = write_set[w_i];
    directory_entry_t new_entry;
    add_accessor(&new_entry, mappings->machine_id);
    DirectoryResult dir_result = dir_client->release(item.key, new_entry);
    tx_dassert(dir_result == DirectoryResult::kSuccess);
  }

  // Clean up invalidation responses.
  for (size_t w_i = 0; w_i < write_set.size(); ++w_i) {
    delete[] inval_resp[w_i];
  }
  delete[] inval_resp;
}
...
forceinline void Tx::abort(coro_yield_t &yield) {
  ...
  for (size_t i = 0; i < write_set.size(); i++) {
    ...
    if (!item.exec_ws_locked || item.is_cached) {
      continue;
    }

    rpc_req_t *req = rpc->start_new_req(
        coro_id, item.rpc_reqtype, item.primary_mn, (uint8_t *)&item.obj->hdr,
        sizeof(uint64_t)); /* Small resps */

    tx_req_arr[req_i] = req;
    req_i++;

    size_t size_req = ds_forge_generic_get_req(
        req, caller_id, item.key, item.keyhash, ds_reqtype_t::unlock);
    req->freeze(size_req);
  }
  ...
  for (size_t i = 0; i < write_set.size(); i++) {
    ...
    if (!item.exec_ws_locked) {
      ...
      Cache *cache = cache_mgr->get_cache(item.rpc_reqtype);
      CacheResult result = cache->abort_write(coro_id, item.keyhash);
      tx_dassert(result == CacheResult::kSuccess);
      continue;
    }
...
bool Tx::validate(coro_yield_t &yield) {
  ...
  for (size_t i = 0; i < read_set.size(); i++) {
    ...
    if (item.is_cached) continue;  // Don't send for cached items, check later.
  ...
  for (size_t i = 0; i < read_set.size(); i++) {
    ...
    tx_dassert(resp_type == ds_resptype_t::get_rdonly_success ||
               ...
               resp_type == ds_resptype_t::get_rdonly_locked || item.is_cached);

    // Re-read the item from the cache if that's where we first got it then
    // check if the versions match.
    if (item.is_cached) {
      size_t out_length;
      Cache *cache = cache_mgr->get_cache(item.rpc_reqtype);
      CacheResult result = cache->get(
          coro_id, item.keyhash, reinterpret_cast<const char*>(&item.key), sizeof(hots_key_t), reinterpret_cast< char *>(&item.obj),
          sizeof(hots_obj_t), &out_length, /*allow_mutation=*/false);
      _unused(result);
      tx_dassert(out_length == sizeof(hots_obj_t));

      if (item.obj->hdr.version != item.exec_rs_version) {
        return false;
      }
      continue;  // Go to next rs item.
    }
    ...
    if (item.exec_rs_exists) {
      ...
      if (resp_type == ds_resptype_t::get_rdonly_success) {
        ...
        tx_dassert(item.obj->hdr.canary == HOTS_VERSION_CANARY);
        tx_dassert(item.obj->val_size ==
                   tx_req_arr[i]->resp_len -
