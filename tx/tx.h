...
typedef DirectoryClient::Result DirectoryResult;
...
class Tx {
 private:
  ...
  CacheManager *cache_mgr;      // To lookup caches based on request type.
  DirectoryClient *dir_client;  // To interact with directory
 ...
 public:
  ...
  Tx(coro_id_t coro_id, Rpc *rpc, Mappings *mappings, Logger *logger,
    ...
    local_log_record = logger->get_log_record(mappings->machine_id, coro_id);
   ...
   */
  forceinline int add_to_read_set(rpc_reqtype_t rpc_reqtype, hots_key_t key,
                                  hots_obj_t *obj) {
    ...
    // Check for the item in the cache first.
    Cache *cache = cache_mgr->get_cache(rpc_reqtype);
    size_t size;
    CacheResult result = cache->get(
        coro_id, item.keyhash, reinterpret_cast<const char *>(&item.key),
        sizeof(item.key), reinterpret_cast<char *>(&item.obj),
        sizeof(hots_obj_t), &size,
        /*allow_mutation=*/false);

    if (result == CacheResult::kSuccess || result == CacheResult::kDeleted) {
      item.is_cached = true;
      item.is_deleted = (result == CacheResult::kDeleted);
    } else {
   ...
   * useful to avoid RPC coalescing in benchmarks.
   */
  forceinline int add_to_write_set(rpc_reqtype_t rpc_reqtype, hots_key_t key,
                                   hots_obj_t *obj,
    ...
    CacheResult result = cache->get(
        coro_id, item.keyhash, reinterpret_cast<const char *>(&item.key),
        sizeof(item.key), reinterpret_cast<char *>(&item.obj),
        sizeof(hots_obj_t), &size, /*allow_mutation=*/false);

    // Item (or tombstone) was in the cache.
    if (result == CacheResult::kSuccess || result == CacheResult::kDeleted) {
      item.is_cached = true;
      item.is_deleted = (result == CacheResult::kDeleted);
    } else {
  ...
  /* tx_execute.h */
  forceinline tx_status_t prepare_tx();
  ...
  static size_t max_pkt_size(int num_keys, size_t val_size) {
    ...
    size_t max_put_req_size =
        num_keys * (sizeof(rpc_cmsg_reqhdr_t) + ds_put_req_size(val_size));

    size_t max_log_record_size =
        sizeof(uint64_t) +          /* Log record header */
