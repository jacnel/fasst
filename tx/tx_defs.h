...
#define tx_dassert(x)               \
  do {                              \
    if (TX_DEBUG_ASSERT) assert(x); \
  } while (0)
#define tx_dassert_msg(x, msg)                    \
  do {                                            \
    if (TX_DEBUG_ASSERT) HOTS_ASSERT_MSG(x, msg); \
  } while (0)
#define tx_stat_inc(x, y)         \
  do {                            \
    if (TX_COLLECT_STATS) x += y; \
  } while (0)
...
struct tx_rwset_item_t {
  ...
  bool is_cached;   // True if the item was already cached when adding to rwset
  bool is_deleted;  // True if the item was deleted when adding to rwset
  uint64_t incarnation;  // Used when preparing/finalizing a cache slot for a
                         // rwset item.
  directory_entry_t dir_entry;  // Read when interacting with the directory.
