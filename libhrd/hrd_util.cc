...
uint64_t hrd_get_interface_id(struct ibv_context *ctx, uint8_t dev_port_id) {
  assert(ctx != nullptr && dev_port_id >= 1);

  union ibv_gid gid;
  if (ibv_query_gid(ctx, dev_port_id, 0, &gid)) {
    printf("HRD: ibv_query_gid on port %d of device %s failed! Exiting.\n",
           dev_port_id, ibv_get_device_name(ctx->device));
    assert(false);
  }

  return gid.global.interface_id;
}

uint64_t hrd_get_subnet_prefix(struct ibv_context *ctx, uint8_t dev_port_id) {
  assert(ctx != nullptr && dev_port_id >= 1);
