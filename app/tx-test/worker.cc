  ...
  retry:
    ...
    if ((stat_tx_tot & M_4_) == M_4_) {
      ...
      printf(
          ...
          "Tx stats = %s\n",
          wrkr_lid, stat_tx_tot / msr_usec, num_reqs / msr_usec,
          num_creqs / msr_usec, stat_tx_tot_usec / stat_tx_tot,
          stat_commit_success / msr_usec,
          (double)stat_commit_success / stat_tx_tot, stat_ex_fail,
          stat_commit_fail, tx->get_stats().c_str());
      fflush(stdout);
      ...
      if (wrkr_lid == 0) {
        ...
        hrd_red_printf(
            "Machine commit tput = %.2f M/s, "
            "req rate = {%.2f M/s, %.2f M/s coalesced}. "
            "Maximum batch latency = %.1f ms. Runtime = %.2f s\n",
            tx_tput_tot, req_rate_tot, creq_rate_tot, max_lat_node / 1000,
            seconds_since_start);
        fflush(stdout);
      }
...
void slave_func(coro_yield_t &yield, int coro_id) {
  ...
  retry:
    ...
    for (int i = 0; i < read_set_size; i++) {
      ...
      if (is_write[i]) {
        primary_mn = tx->add_to_write_set(RPC_MICA_REQ, key_arr[i], &obj_arr[i],
                                          tx_write_mode_t::update);
      } else {
        primary_mn = tx->add_to_read_set(RPC_MICA_REQ, key_arr[i], &obj_arr[i]);
      }
...
void init_rpc() {
  ...
  struct rpc_args _rpc_args = rpc_args(
      wrkr_gid, wrkr_lid, num_workers, workers_per_machine, num_coro,
...
void run_thread(struct thread_params *params) {
  ...
  mappings = new Mappings(wrkr_gid, num_machines, workers_per_machine,
                          num_backups, use_lock_server);
  logger = new Logger(wrkr_gid, wrkr_lid, num_machines, num_coro);

  cache_mgr = new CacheManager();
  cache_mgr->register_cache(RPC_MICA_REQ, params->cache);
  ...
  if (!use_lock_server || !mappings->am_i_lock_server) {
    ...
    zg = new ::mica::util::ZipfGen(num_keys_global, zipf_theta,
                                   zipf_seed & zipf_seed_mask);
    ...
    for (int repl_i = 0; repl_i < mappings->num_replicas; repl_i++) {
      ...
      ds_fixedtable_populate(
          params->fixedtable[repl_i], num_keys_global, val_size, mappings,
          repl_i, wrkr_gid, /* = partition ID */
    ...
    for (int repl_i = 0; repl_i < mappings->num_replicas; repl_i++) {
      rpc->register_rpc_handler(RPC_MICA_REQ + repl_i,
                                ds_fixedtable_rpc_handler,
                                (void *)params->fixedtable[repl_i]);
    }
    ...
    for (int coro_i = 0; coro_i < num_coro; coro_i++) {
      if (coro_i == RPC_MASTER_CORO_ID) {
        coro_arr[coro_i] =
            coro_call_t(bind(master_func, _1, coro_i), attributes());
        // attributes(fpu_not_preserved));
      } else {
        if (read_set_size == 1 && write_percentage == 0) {
          coro_arr[coro_i] = coro_call_t(
              bind(slave_func_single_read, _1, coro_i), attributes());
          // attributes(fpu_not_preserved));
        } else {
          coro_arr[coro_i] =
              coro_call_t(bind(slave_func, _1, coro_i), attributes());
          // attributes(fpu_not_preserved));
        }
      }
    }
    ...
    rpc->register_rpc_handler(RPC_LOCKSERVER_REQ, lockserver_rpc_handler,
                              (void *)lockserver);

    coro_call_t lockserver_coro = coro_call_t(lockserver_func, attributes());
    // attributes(fpu_not_preserved));
    lockserver_coro();
#endif
  }
