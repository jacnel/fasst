...
static FixedTable *ds_fixedtable_init(const char *config_filepath,
                                      size_t val_size, int bkt_shm_key,
                                      bool is_primary) {
  ds_do_checks();
  ...
  if (!table_config.get("concurrent_read").get_bool() ||
    ...
    fprintf(stderr, "HoTS Error: FixedTable only supports CRCW. Exiting.\n");
    exit(-1);
  }

  FixedTableConfig::Alloc *alloc =
      new FixedTableConfig::Alloc(config.get("alloc"));

  FixedTable *table = new FixedTable(config.get("table"), val_size, bkt_shm_key,
                                     alloc, is_primary);
...
/* Destroy the table */
static void ds_fixedtable_free(FixedTable *table) { delete table; }
 ...
 * @val_size is the application-level opaque buffer size.
 *
 * If use_partitions is false, all keys in the range are added to this
 * datastore. In this case, invalid mappings and repl_i should be passed.
 *
 ...
 */
static void ds_fixedtable_populate(FixedTable *table, size_t num_keys,
                                   size_t val_size, Mappings *mappings,
                                   int repl_i, int wrkr_gid,
                                   bool use_partitions, int const_val = -1) {
  if (!use_partitions) {
  ...
  printf(
      "HoTS: Populating table %s for worker %d as replica %d. "
      "(%lu keys, val size = %lu)\n",
  ...
  for (size_t i = 0; i < num_keys; i++) {
    ...
    MicaResult out_result =
        table->set_spinlock(wrkr_gid, keyhash, key, (char *)obj.val);

    if (out_result != MicaResult::kSuccess) {
      fprintf(stderr,
              "HoTS: Failed to populate table %s for worker %d. "
  ...
  printf(
      "HoTS: Done populating table %s for worker %d. Added %lu of %lu keys\n",
