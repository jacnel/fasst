...
class Mappings {
 public:
  ...
  Mappings(int wrkr_gid, int num_machines, int workers_per_machine,
    ...
    tot_directory_machines = num_machines;

    am_i_lock_server =
        use_lock_server ? (machine_id == num_machines - 1) : false;
  }

  forceinline int get_directory_mn(uint64_t keyhash) {
    /*
