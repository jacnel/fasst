#pragma once
#ifndef MICA_TABLE_CTABLE_IMPL_DEBUG_H_
#define MICA_TABLE_CTABLE_IMPL_DEBUG_H_

namespace mica {
namespace table {
template <class StaticConfig>
void CTable<StaticConfig>::print_bucket(const Bucket* bucket) const {
  printf("<bucket %zx>\n", (size_t)bucket);
...
template <class StaticConfig>
void CTable<StaticConfig>::print_buckets() const {
...
template <class StaticConfig>
void CTable<StaticConfig>::print_stats() const {
  if (StaticConfig::kCollectStats) {
    ...
    printf("callbacks_made:         %10zu\n", stats_.callbacks_made);
    printf("cache_invalidated:      %10zu\n", stats_.cache_invalidated);
  }
}

template <class StaticConfig>
void CTable<StaticConfig>::reset_stats(bool reset_count) {
  if (StaticConfig::kCollectStats) {
...
template <class StaticConfig>
void CTable<StaticConfig>::stat_inc(size_t Stats::*counter) const {
  if (StaticConfig::kCollectStats) __sync_add_and_fetch(&(stats_.*counter), 1);
}

template <class StaticConfig>
void CTable<StaticConfig>::stat_dec(size_t Stats::*counter) const {
