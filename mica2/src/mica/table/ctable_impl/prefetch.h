#pragma once
#ifndef MICA_TABLE_CTABLE_IMPL_PREFETCH_H_
#define MICA_TABLE_CTABLE_IMPL_PREFETCH_H_

namespace mica {
namespace table {
template <class StaticConfig>
void CTable<StaticConfig>::prefetch_table(uint64_t key_hash) const {
...
template <class StaticConfig>
void CTable<StaticConfig>::prefetch_pool(uint64_t key_hash) const {
  uint32_t bucket_index = calc_bucket_index(key_hash);
  const Bucket* bucket = buckets_ + bucket_index;
