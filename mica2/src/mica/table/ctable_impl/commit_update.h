#pragma once
#ifndef MICA_TABLE_CTABLE_IMPL_COMMIT_UPDATE_H_
#define MICA_TABLE_CTABLE_IMPL_COMMIT_UPDATE_H_

namespace mica {
namespace table {

template <class StaticConfig>
Result CTable<StaticConfig>::commit_update(uint32_t caller_id,
                                           uint64_t key_hash, const char* key,
  ...
  // Update expects that the item exists in the cache.
  ...
  if (old_item_size >= new_item_size) {
    ...
    update_item_value(item, value, (uint32_t)value_length, /*deleted=*/false);
    pool_->unlock();
    unlock_bucket(bucket, caller_id);
  ...
  set_updated_item(new_item, key_hash, key, (uint32_t)key_length, value,
