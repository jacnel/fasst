#pragma once
#ifndef MICA_TABLE_CTABLE_IMPL_BUCKET_H_
#define MICA_TABLE_CTABLE_IMPL_BUCKET_H_

namespace mica {
namespace table {
template <class StaticConfig>
uint16_t CTable<StaticConfig>::get_tag(uint64_t item_vec) {
  return static_cast<uint16_t>(item_vec >> 48);
}

template <class StaticConfig>
uint64_t CTable<StaticConfig>::get_item_offset(uint64_t item_vec) {
  return item_vec & Bucket::kItemOffsetMask;
}

template <class StaticConfig>
uint64_t CTable<StaticConfig>::make_item_vec(uint16_t tag,
                                             uint64_t item_offset) {
  return ((static_cast<uint64_t>(tag) << 48) | item_offset);
}

template <class StaticConfig>
uint32_t CTable<StaticConfig>::calc_bucket_index(uint64_t key_hash) const {
  // 16 is from Bucket::kTagMask's log length
  return static_cast<uint32_t>(key_hash >> 16) & num_buckets_mask_;
}

template <class StaticConfig>
bool CTable<StaticConfig>::has_extra_bucket(const Bucket* bucket) {
...
template <class StaticConfig>
const typename CTable<StaticConfig>::Bucket*
CTable<StaticConfig>::get_extra_bucket(uint32_t extra_bucket_index) const {
...
template <class StaticConfig>
typename CTable<StaticConfig>::Bucket* CTable<StaticConfig>::get_extra_bucket(
...
template <class StaticConfig>
bool CTable<StaticConfig>::alloc_extra_bucket(Bucket* bucket) {
...
template <class StaticConfig>
void CTable<StaticConfig>::free_extra_bucket(Bucket* bucket) {
  assert(has_extra_bucket(bucket));
...
template <class StaticConfig>
void CTable<StaticConfig>::fill_hole(Bucket* bucket, size_t unused_item_index) {
  while (true) {
...
template <class StaticConfig>
size_t CTable<StaticConfig>::get_empty(Bucket* bucket,
...
template <class StaticConfig>
size_t CTable<StaticConfig>::get_empty_or_oldest(Bucket* bucket,
                                                 Bucket** located_bucket) {
...
template <class StaticConfig>
size_t CTable<StaticConfig>::find_item_index(
...
template <class StaticConfig>
size_t CTable<StaticConfig>::find_item_index(Bucket* bucket, uint64_t key_hash,
...
template <class StaticConfig>
size_t CTable<StaticConfig>::find_same_tag(
...
template <class StaticConfig>
size_t CTable<StaticConfig>::find_same_tag(Bucket* bucket, uint16_t tag,
                                           Bucket** located_bucket) {
...
template <class StaticConfig>
void CTable<StaticConfig>::cleanup_all() {
  if (std::is_base_of<::mica::pool::CircularLogTag,
                      typename Pool::Tag>::value) {
