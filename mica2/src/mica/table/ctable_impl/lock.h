...
void CTable<StaticConfig>::lock_bucket(Bucket* bucket) {
  if (StaticConfig::kConcurrent) {
    if (concurrent_access_mode_ == 1) {
      assert((*(volatile uint64_t*)&bucket->version & 1U) == 0U);
      ...
      while (true) {
        ...
        uint64_t new_v = v | 1U;
        if (__sync_bool_compare_and_swap((volatile uint64_t*)&bucket->version,
                                         v, new_v))
          break;
      }
    }
...
template <class StaticConfig>
void CTable<StaticConfig>::unlock_bucket(Bucket* bucket) {
  if (StaticConfig::kConcurrent) {
...
template <class StaticConfig>
void CTable<StaticConfig>::lock_extra_bucket_free_list() {
  if (StaticConfig::kConcurrent) {
...
template <class StaticConfig>
void CTable<StaticConfig>::unlock_extra_bucket_free_list() {
  if (StaticConfig::kConcurrent) {
    if (concurrent_access_mode_ == 2) {
      ::mica::util::memory_barrier();
