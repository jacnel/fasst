#pragma once
#ifndef MICA_TABLE_CTABLE_IMPL_ITEM_H_
#define MICA_TABLE_CTABLE_IMPL_ITEM_H_

namespace mica {
namespace table {
template <class StaticConfig>
uint32_t CTable<StaticConfig>::get_key_length(uint32_t kv_length_vec) {
  return kv_length_vec >> 24;
}

template <class StaticConfig>
uint32_t CTable<StaticConfig>::get_value_length(uint32_t kv_length_vec) {
  return kv_length_vec & Item::kValueMask;
}

template <class StaticConfig>
uint32_t CTable<StaticConfig>::make_kv_length_vec(uint32_t key_length,
                                                  uint32_t value_length) {
  return (key_length << 24) | value_length;
}

template <class StaticConfig>
uint16_t CTable<StaticConfig>::calc_tag(uint64_t key_hash) {
...
template <class StaticConfig>
bool CTable<StaticConfig>::compare_keys(const char* key1, size_t key1_len,
