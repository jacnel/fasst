#pragma once
#ifndef MICA_TABLE_CTABLE_IMPL_CACHE_H_
#define MICA_TABLE_CTABLE_IMPL_CACHE_H_

namespace mica {
namespace table {
template <class StaticConfig>
Result CTable<StaticConfig>::cache(uint64_t key_hash, const char* key,
  ...
  if (expected_incarnation !=
      *(const_cast<volatile uint32_t*>(bucket->incarnation))) {
  ...
  if (located_bucket->item_vec[item_index] != 0) {
    // Perform the callback on the evicted item
    uint64_t old_item_offset =
        get_item_offset(located_bucket->item_vec[item_index]);
    Item* old_item = reinterpret_cast<Item*>(pool_->get_item(old_item_offset));
    if (old_item->modified) {
      // Execute eviction callback on modified items only.
      size_t old_item_key_length = get_key_length(old_item->kv_length_vec);
      size_t old_item_value_length = get_value_length(old_item->kv_length_vec);
      callback_(old_item->data, old_item_key_length,
                old_item->data + ::mica::util::roundup<8>(old_item_key_length),
                old_item_value_length);
