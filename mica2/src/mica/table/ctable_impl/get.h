#pragma once
#ifndef MICA_TABLE_CTABLE_IMPL_GET_H_
#define MICA_TABLE_CTABLE_IMPL_GET_H_
...
Result CTable<StaticConfig>::get(uint64_t key_hash, const char* key,
  ...
  while (true) {
    uint64_t version_start = read_version_begin(bucket);
    ...
    if (invalid_value_length || !Specialization::is_valid(pool_, item_offset)) {
      ...
      if (allow_mutation) {
        auto mut_this = const_cast<CTable<StaticConfig>*>(this);
