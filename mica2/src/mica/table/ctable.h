#pragma once
#ifndef MICA_TABLE_CTABLE_H_
#define MICA_TABLE_CTABLE_H_
...
#include "mica/table/ctable_impl/specialization.h"

// Configuration file entries for CTable:
//
...
struct BasicLossyCTableConfig : public BasicCTableConfig {
  ...
  // Supported: 7, 15, 31.
  static constexpr size_t kBucketSize = 14;
...
template <class StaticConfig = BasicLossyCTableConfig>
class CTable : public TableInterface {
 public:
  typedef typename StaticConfig::Alloc Alloc;
  typedef typename StaticConfig::Pool Pool;
  typedef typename StaticConfig::EvictionCallback EvictionCallback;

  // Assumes eviction is enabled and that we are using a circular log.
  static_assert(StaticConfig::kEviction == true);
  static_assert(
      std::is_base_of<::mica::pool::CircularLogTag, typename Pool::Tag>::value);
  ...
  // ctable_impl/init.h
  CTable(const ::mica::util::Config& config, Alloc* alloc, Pool* pool,
 ...
 private:
  typedef CTablePoolSpecialization<typename Pool::Tag> Specialization;
  ...
  static_assert((StaticConfig::kBucketSize == 6 && sizeof(Bucket) == 64) ||
                    (StaticConfig::kBucketSize == 14 &&
                     sizeof(Bucket) == 128) ||
                    (StaticConfig::kBucketSize == 30 && sizeof(Bucket) == 256),
  ...
  struct Stats {
    ...
    size_t callbacks_made;
    size_t cache_invalidated;
  };

  // ctable_impl/bucket.h
  ...
  uint32_t get_bucket_version(uint64_t key_hash) const;
