...
#define DIR_DEBUG_ASSERT 0    // directory.h
#define CACHE_DEBUG_ASSERT 0  // cache.h
...
#define hots_obj_size(val_size) \
  (sizeof(hots_obj_t) - HOTS_MAX_VALUE + (val_size))
...
struct hots_mbuf_t {
  ...
  inline size_t length() { return (size_t)(cur_buf - alloc_buf); }

  inline void reset() { cur_buf = alloc_buf; }
  ...
  inline bool is_valid() {
     ...
     */
    return (
        alloc_buf != NULL && alloc_len > 0 && cur_buf != NULL &&
        cur_buf >= alloc_buf && ((uint64_t)alloc_buf) % sizeof(uint64_t) == 0 &&
        ((uint64_t)cur_buf) % sizeof(uint32_t) == 0 && length() <= alloc_len);

    return true; /* XXX: Why do I need this to compile? */
  }

  inline size_t available_bytes() { return (alloc_len - length()); }
};
